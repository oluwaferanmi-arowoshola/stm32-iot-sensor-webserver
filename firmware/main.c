...
#define SSID2     "mavnet-psk"
#define PASSWORD "aph9Aech"
...
static  WIFI_Status_t SendWebPage(uint8_t ledIsOn, uint8_t temperature, float humidity);
...
int wifi_server(void)
{
 ...
 /*Initialize and use WIFI module */
  if(WIFI_Init() ==  WIFI_STATUS_OK)   // To-do: initialize Wifi
  {
  ...
  {
	  LOG(("ERROR : CANNOT INIT es-wifi module \n"));
    return -1;
  }

   LOG(("\nConnecting to %s , %s\n",SSID2,PASSWORD));
    if( WIFI_Connect(SSID2, PASSWORD, WIFI_ECN_WPA2_PSK) == WIFI_STATUS_OK)		// To-do: connect wifi
    {
      //if(WIFI_GetIP_Address(IP_Addr, sizeof(IP_Addr)) == WIFI_STATUS_OK)
  	  if( WIFI_GetIP_Address(IP_Addr) == WIFI_STATUS_OK)		// To-do: Get dynamic IP address
      {
  ...
  if (WIFI_STATUS_OK == WIFI_ReceiveData(SOCKET, resp, 1000, &respLen, WIFI_READ_TIMEOUT))
  {
   ...
   if( respLen > 0)
   {
      if(strstr((char *)resp, "GET")) /* GET: put web page */
      {
        ...
        if(SendWebPage(LedState, temp, humidity_value) != WIFI_STATUS_OK)
        {
...
static WIFI_Status_t SendWebPage(uint8_t ledIsOn, uint8_t temperature, float humidity)
{
  ...
  // To-do: Create the similar entry as Temp for humidity.
  sprintf((char *)humiditystr, "%.2f", humidity);
  strcat((char *)http, (char *)"<p><form method=\"POST\"><strong>Humidity: <input type=\"text\" value=\"");
  strcat((char *)http, (char *)humiditystr);
  strcat((char *)http, (char *)"\"> <sup>% RH</sup>");
